...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `jetstream` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `triplewhale` with the `ORG` of your provider.
   - replace all instances of the `github.com/Triple-Whale/pulumi-jetstream` repository with the `REPOSITORY` location
...
├── config.yaml
├── metadata.json
├── pulumi-jetstream_darwin_amd64_v1
│   └── pulumi-resource-jetstream
├── pulumi-jetstream_darwin_arm64
│   └── pulumi-resource-jetstream
├── pulumi-jetstream_linux_amd64_v1
│   └── pulumi-resource-jetstream
├── pulumi-jetstream_linux_arm64
│   └── pulumi-resource-jetstream
├── pulumi-jetstream_windows_amd64_v1
│   └── pulumi-resource-jetstream.exe
└── pulumi-jetstream_windows_arm64
    └── pulumi-resource-jetstream.exe
```
